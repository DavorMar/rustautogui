...
pub fn fft_ncc(
    ...
    let (
        ...
        template_width, 
        template_height,
        padded_size) = prepared_data;


    let mut planner = FftPlanner::<f32>::new();
    let fft: std::sync::Arc<dyn Fft<f32>> = planner.plan_fft_forward((padded_size * padded_size) as usize);
    ...
    let mut image_padded: Vec<Complex<f32>> = vec![Complex::new(0.0, 0.0); (padded_size * padded_size) as usize];
    for dy in 0..image_height {
        for dx in 0..image_width {
            let image_pixel_value = zero_mean_image[dy as usize][dx as usize];
            image_padded[ dy as usize * *padded_size as usize + dx as usize] = Complex::new(image_pixel_value, 0.0);
        }
    }


    // conver image into frequency domain
    let ifft: std::sync::Arc<dyn Fft<f32>> = planner.plan_fft_inverse((padded_size * padded_size) as usize);
    fft.process(&mut image_padded);
 
    // calculate F(image) * F(template).conjugate
    let product_freq: Vec<Complex<f32>> = image_padded.iter()
    ...
    println!("{}, {}", image_height, template_height);
...
pub fn prepare_template_picture (
    ...
    let mut template_padded: Vec<Complex<f32>> = vec![Complex::new(0.0, 0.0); (padded_size * padded_size) as usize];
    for dy in 0..template_height {
        for dx in 0..template_width {
            let template_pixel_value = zero_mean_template[dy as usize][dx as usize];
            template_padded[dy as usize * padded_size as usize +  dx as usize] = Complex::new(template_pixel_value, 0.0);
        }
    }
    ...
    let template_conj_freq: Vec<Complex<f32>> = template_padded.iter().map(|&val| val.conj()).collect();
    let return_value = (template_conj_freq, template_sum_squared_deviations, template_width, template_height, padded_size);
    return_value
}